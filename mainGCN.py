...
pd_i = pd.read_csv('i.csv')
n_graph_nodes = pd_i['0'].max()
print(n_graph_nodes)
...
if args.graph == 'default':
    ...
    # load customized graph weight matrix, W: np.ndrray
    W = weight_matrix(pjoin('./dataset', args.graph))

# Calculate graph kernel
L = scaled_laplacian(W)
# Alternative approximation method: 1st approx - first_approx(W, n). 这两种方法就不需要改了。
...
n_train, n_val, n_test = 34, 5, 5 # 原始设置是34 5 5 ，这里调小一点试试
